...
const Office = () => {
    ...
    const filteredOfficeProds = prods.filter(
      prod => prod.clothe === 'office' && prod.category === "men's"
    )

    const soldThisWeek = filteredOfficeProds.filter(prod => prod.id % 2 === 0)

    const cheapOnes = filteredOfficeProds.filter(prod => prod.price < 100)
