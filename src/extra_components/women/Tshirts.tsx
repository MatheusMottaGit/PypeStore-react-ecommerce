...
const Tshirts = () => {
    ...
    const filteredShirtsProds = prods.filter( 
      prod => prod.clothe === 't-shirt' && prod.category === "women's"
    )

    const soldThisWeek = filteredShirtsProds.filter(prod => prod.id % 2 === 0)

    const cheapOnes = filteredShirtsProds.filter(prod => prod.price < 100)
